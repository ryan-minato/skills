...
- [ ] 3.3 `.agents/knowledge/{github-checks,github-settings,harness-maintenance,agent-authority,github-workflow}.md` and `ARCHITECTURE.md` carry the two `spec / *` job rows, the privileged-read rule, the mirror rows, the label rows, and no bypass requirement; verify `just validate` and a read-through

## 4. Proofs

- [x] 4.1 Catalog scaffold proofs (validate, marketplace drift, symlink listing, README pair reads, spec-sync)
- [ ] 4.2 Automation proofs (files parse, pinned SHAs, no push and no head read in any `spec / *` workflow, `just spec-check` draft and ready verdicts, mirror diff, dry-run label list, the `spec/extra` negative case)
- [ ] 4.3 Contract and project skill proofs (a clean-context read states the draft's first content, when the request is marked ready, who archives and when, and what the archive freezes)

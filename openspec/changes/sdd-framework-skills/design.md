...
- Platform facts the automation is designed on (verified 2026-09-17
  against the GitHub and GitLab documentation): a fork's `pull_request`
  run holds a read-only token and no secrets, and the setting that would
  grant it write exists for private repositories only, so labelling or
  replying on an external contribution needs a privileged trigger;
  `issue_comment` runs the default-branch workflow file; comments the
  platform token posts create no workflow run; the platform token cannot
  push to a fork's branch; GitLab has no pipeline source for a note or a
  label change, and running a fork's pipeline in the parent project runs
  the fork's configuration with the parent's variables.
...
| OSW: GitHub install; Asked for an archive bot; Ready with an unarchived change; GitLab install | o5 — fixture repository with a `checks` workflow and gate: "install the automation" | check job joins the gate's `needs:` (critical); the two privileged workflows + labels + script present; nothing installed pushes; the label sync named | 4/4 | sonnet | transcript + file diff | worktree |

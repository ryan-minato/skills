...
#### Scenario: A commit lands after the freeze
- **WHEN** the agent is about to push, or to hand the request over, and a commit exists after the archive commit
- **THEN** it says the freeze is stale, names the commits, and asks whether the deliberation reopens rather than treating the earlier closing as still standing

...
- [x] 2.1 Confirm `git diff --stat origin/main...HEAD -- scripts .github skills/meta skills/machine-learning` is empty; verify `just validate`
...
- [x] 4.1 Run `just check`, write the results to the pull request's Validation section linking this plan, and archive this change in-request beside `ml-standard-alignment`, then `just spec-validate`

...
**Reason**: the script is harness automation for the OpenSpec archive step and is consumed by the harness builders; it belongs to the spec workflow builder, not to the methodology skill, and a builder must not depend on a file outside its catalog.
**Migration**: the script ships unchanged from `meta-spec-workflow`; a project that copied it keeps its copy and updates its recorded origin.

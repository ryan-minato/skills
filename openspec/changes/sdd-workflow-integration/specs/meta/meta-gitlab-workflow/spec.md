...
The generated project skill SHALL, under the combined shape, take a work item that has no specification by committing the change record to a draft merge request first and waiting for the gate owner's recorded approval, and SHALL, under the split shape, escalate a work item whose specification merge request is not merged.
...
- **THEN** the generated skill directs it to publish the change record on a draft merge request and wait for the approval comment rather than escalating
...
- **WHEN** a merge request body is built from the adapted template and every item is ticked
...
Under the split shape, the specification merge request SHALL reference the work item without closing it, and the final implementation merge request SHALL close it.

#### Scenario: Split shape
- **WHEN** the contract records the split shape and the generated skill opens the specification merge request

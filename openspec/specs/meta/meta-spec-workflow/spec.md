...
The project skill steps the builder inserts SHALL, under the conversational mode, make the agent read the request's comments and review threads with their resolution state when the gate owner closes either deliberation, list unresolved threads and requested adjustments missing from the package, ask the gate owner to confirm them, and start the task list and implementation only when nothing is open or the open items are confirmed.
...
- **THEN** the builder asks once whether anyone outside the conversation must verify the acceptance, recommends the conversational mode on the complete package for both gates when nobody must, and records the answer

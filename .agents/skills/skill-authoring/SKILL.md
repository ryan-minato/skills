...
4. The `change-workflow` skill must already have placed the worktree on a
   dedicated branch and, for a behavior change, created and had approved the
   OpenSpec change `openspec/changes/<slug>/` with a delta spec for the
...
2. Repo path references are allowed here, but all other quality standards
   still apply.
3. The `openspec-*` directories there are generated by the OpenSpec CLI:
   never edit, lint-fix, or author them by hand — `just spec-sync` after a
   version bump in the `justfile` is the only way they change.

## Removing, moving, or renaming a skill

First trace every reference: `git grep -n <old-name>` across `skills/`,
`.agents/`, `.github/`, `openspec/`, and the root documents finds other
skills' handoffs, catalog `CONTEXT.md` routing, project skills, and specs
that name it. Fix all of them in the same change so no handoff points at a
name that no longer exists; a renamed skill's spec domain moves with it
through its OpenSpec change. Then remove/update the symlink in
`.agents/skills/`, the catalog README.md + README.zh.md entries, and run
`just gen-marketplace` to resync `.claude-plugin/marketplace.json` (remove a
plugin entry by hand only if its catalog became empty; a catalog left empty
follows the catalog row in `.agents/knowledge/harness-maintenance.md`). The
validator catches the mechanical part; it cannot see a stale name inside
prose.

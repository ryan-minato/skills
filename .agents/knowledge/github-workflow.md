...
| `spec/archive` | Applied by a human with triage access to a pull request; the `spec / archive` workflow archives the pull request's completed changes, pushes, and removes it (a fork gets the local commands instead). | Archiving stays a manual sequence every author repeats. |
| `spec/archived` / `spec/unarchived`, `spec/not-started` / `spec/in-progress` / `spec/done` | Two axes the `spec / labels` and `spec / archive` workflows derive from the pull request's related OpenSpec changes; never set by hand; absent on a pull request with no related change. | A reviewer cannot see from the list whether a pull request is archived or how far its tasks are. |

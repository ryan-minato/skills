...
| Ruleset bypass actors | none: nothing pushes to `main` outside a pull request. The archive bot (`spec-archive.yml`) pushes to the pull request's own branch with `GITHUB_TOKEN`, so no bypass is needed; after each of its pushes the maintainer clicks **Approve workflows to run** on the pull request (`.agents/knowledge/github-checks.md`). Live state: none (the 2026-09-10 attempt to add the Actions app was refused by the REST API on this user-owned repository and is no longer wanted) | enforced | same readback (`bypass_actors`) | anything is ever granted a bypass; a second maintainer joins |

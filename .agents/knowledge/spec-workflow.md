...
| `openspec/changes/<slug>/` | One change in flight: `proposal.md`, delta specs under `specs/<catalog>/<skill-name>/` (none for a repository change), `design.md` (placement, description, sync obligations, and the verification plan written before implementation), `tasks.md`. `<slug>` equals the branch slug. | the change's author, through the OpenSpec skills |
...
2. **Approved** — the agent stops at the draft. The maintainer discusses
   the proposal and delta specs on the pull request, directs record
   changes in conversation (each pushed through the publish gate), and
   declares the discussion closed in conversation. The agent then reads
   the pull request's comments and review threads with their resolution
   state, lists every unresolved thread, every requested adjustment the
   record does not carry, and every contradiction, asks the maintainer to
   confirm the open items, and proceeds only when nothing is open or the
   open items are confirmed; the closing is noted on the draft's
   `Approval:` line. The review examines the outcome description — goals
   and scope, terminology, behavior, invariants, constraints, states,
   interface and data contracts, edge cases, security, acceptance — never
   `design.md` or `tasks.md`, which the author finishes after the closing.
   A pull request review approval is not the record: later pushes dismiss
   it. An agent never closes the discussion on a change it wrote.
...
on that draft by closing its discussion, design and tasks follow the
reconciled closing, and marking it ready requests implementation review. A change that also needs harness work
...
`meta-spec-workflow` skill's script — validating strictly and
...
- A pull request names its change in its Specification section: a
  ...
  (`specification` until the discussion is closed, `implementation` after),
...
- The level, the approval owner or mode, the change request shape, or the
  archive mode changes; the Actions bypass is granted or revoked.

#!/usr/bin/env python3
"""Read-only GitHub reader: the last-resort REST fallback tier.

This is the last-resort read tier: prefer a connected MCP server or an
authenticated gh CLI when either is available, and keep requests to the
minimum the task needs — anonymous access is tightly rate-limited
(60 requests/hour per IP; search 10/minute).
...
def _cli_hint() -> None:
    if shutil.which("gh"):
        print(
            "hint: the gh CLI is installed. If `gh auth status` exits 0, "
            "prefer gh commands over this script (authenticated: 5,000 "
            "requests/hour vs 60/hour anonymous). Either way, keep REST "
            "calls to the minimum the task needs.",
            file=sys.stderr,
        )
...
def _headers(accept: str, auth: bool) -> dict:
    headers = {
        ...
        "User-Agent": "meta-github-workflow-rest-read",
    }
    token = _token() if auth else None  # pragma: allowlist secret
    if token:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Last-resort read tier: prefer a connected MCP server or an "
            "authenticated gh CLI when available, and minimize requests — "
            "anonymous access is limited to 60 requests/hour (search: "
            "10/minute)."
        ),
    )

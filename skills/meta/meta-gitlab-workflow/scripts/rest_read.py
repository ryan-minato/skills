...
connected MCP server when either is available, and keep requests to the
minimum the task needs — unauthenticated access is rate-limited by the
host (an HTTP 429 reports the Retry-After delay; stop, never retry into
the limit).
...
def _cli_hint() -> None:
    if shutil.which("glab"):
        print(
            "hint: the glab CLI is installed. If `glab auth status "
            "--hostname HOST` exits 0 for the target host, prefer glab "
            "commands over this script (authenticated, higher rate "
            "limits). Either way, keep REST calls to the minimum the "
            "task needs — unauthenticated requests are rate-limited by "
            "the host.",
            file=sys.stderr,
        )
...
def _headers() -> dict:
    headers = {
        ...
        "User-Agent": "meta-gitlab-workflow-rest-read",
    }
    token = _token()  # pragma: allowlist secret
    if token:
...
def _resolve_host(value: str | None) -> str:
    host = (value or os.environ.get("GITLAB_HOST") or os.environ.get("GL_HOST") or "").strip()
    if "://" in host:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "unauthenticated access is rate-limited by the host (429 "
            "reports Retry-After; stop, never retry into the limit)."
        ),
    )
    ...
    def common(p: argparse.ArgumentParser, project=True, limit=True) -> None:
        ...
        p.add_argument(
            "--host",
            help="GitLab host (default: GITLAB_HOST, then GL_HOST; never assumed)",
        )
        if limit:
...
def main() -> None:
    ...
    if not host:
        parser.error(
            "cannot determine the target GitLab host; pass --host HOST or set "
            "GITLAB_HOST/GL_HOST after resolving it from the project remote"
        )

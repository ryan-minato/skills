...
- host:      the GitLab host being probed and where it came from
             (--hostname flag, GITLAB_HOST, or GL_HOST). The caller must
             resolve the project host first; this tool never assumes GitLab.com.
...
def resolve_host(flag_value: str | None) -> dict:
    """Resolve the host to probe: flag > GITLAB_HOST > GL_HOST."""
    if flag_value:
    ...
    return {"host": None, "source": "missing"}
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "GL_HOST. Exit codes: 0 = probes ran (missing "
    ...
    if host is None:
        parser.error(
            "cannot determine the target GitLab host; derive it from the project "
            "remote, then pass --hostname HOST or set GITLAB_HOST/GL_HOST"
        )

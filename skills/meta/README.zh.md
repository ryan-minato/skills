...
| [meta-workflow-design](meta-workflow-design/) | 与人类开发者在平台无关的模型中共同设计项目的管理模型——项目真正配得上的工作跟踪语义、贴合实际交付方式的 workflow profile、overlay 与变更传播风险——并以项目自身平台的词汇落地为 workflow 文件，供平台构建技能实现而非重新决定。 |
| [meta-spec-workflow](meta-spec-workflow/) | 以框架无关的方式分两个阶段为项目配置规格驱动开发规则：与用户敲定规格级别与方法家族、变更请求形态、两道审批门各自的方式与审批包（规格加上有必要时的受约束 design；默认两道门都在对话中收口，只有当对话之外的人必须自行核验时才采用记录式批准）、存档执行者（由人在 request 内、在实现评议收口后执行）与规格范围，为每个事实保留唯一来源，以项目自身平台的词汇落地规格契约，并把工具接入与自动化交给 `sdd` catalog 的框架 skill；待平台底座交付后，再填充其扩展槽——请求模板的规格块与检查项、intake 字段、项目技能的领取、草稿、收口核对与收尾步骤，以及知识节。 |
...
| [meta-github-workflow](meta-github-workflow/) | 为 GitHub.com 或 GitHub Enterprise 构建或系统修复完整、与开发范式无关的 GitHub 仓库生命周期 harness，围绕 PR 回路设计并带有登记在册、供范式构建技能事后填充的扩展槽：intake 表单与 Discussions 路由、在默认集上扩展的标签、tracking issue 与 milestone、经关联分支的早期 draft PR、Actions 质量门与社区自动化、ruleset、CODEOWNERS、标签驱动发布说明的 release、注册表、可选 Projects 与 ML 实验记录，以及持久项目 agent 工作流。 |
| [meta-gitlab-workflow](meta-gitlab-workflow/) | 为 gitlab.com 或自托管实例构建或系统修复完整、与开发范式无关的 GitLab 项目生命周期 harness，带有登记在册、供范式构建技能事后填充的扩展槽：规划、work item 与早期 draft MR、社区文件、CI/CD、治理、安全、Wiki、发布、部署、注册表、可选 MLOps 及持久项目 agent 工作流。 |

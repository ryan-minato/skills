...
| [meta-spec-workflow](meta-spec-workflow/) | 分两个阶段为项目配置规格驱动开发规则：与用户敲定规格级别与工具（Spec-Kit、OpenSpec、Kiro 或仓库内文档）、变更请求形态、审批方式（默认在草稿上讨论收口）、存档方式与规格范围，安装或接入工具布局，为每个事实保留唯一来源，并以项目自身平台的词汇落地规格契约；待平台底座交付后，再填充其扩展槽——请求模板的规格块与检查项、intake 字段、项目技能的领取、草稿、收口核对与收尾步骤、存档 workflow 或 job（附带存档已完成 OpenSpec 变更的脚本），以及检查命令中的校验器。 |

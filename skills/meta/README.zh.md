...
| [meta-spec-workflow](meta-spec-workflow/) | 以框架无关的方式分两个阶段为项目配置规格驱动开发规则：与用户敲定规格级别与方法家族、变更请求形态、审批方式与审批包（规格加上有必要时的受约束 design；默认在草稿上讨论收口）、存档执行者（在 request 内手工，或由该工具的框架 skill 安装的自动化）与规格范围，为每个事实保留唯一来源，以项目自身平台的词汇落地规格契约，并把工具接入与自动化交给 `sdd` catalog 的框架 skill；待平台底座交付后，再填充其扩展槽——请求模板的规格块与检查项、intake 字段、项目技能的领取、草稿、收口核对与收尾步骤，以及知识节。 |

...
| [training-instrumentation](training-instrumentation/) | Design what a training run emits: outcome, optimization, and systems layers joined by shared correlation keys; metrics, stage-level traces, and scheduled profiles kept separate; a minimal signal set by priority with sampling frequencies and bounded labels; model-health metrics beyond loss and model-family signals; symptom-first alerts relative to a healthy baseline with persistence and composite rules under five severities; dashboards; cluster-scale collection, sampling, retention, and capacity. |
| [training-diagnosis](training-diagnosis/) | Diagnose a run that misbehaves: locate the first anomaly across signals, walk a top-down ladder from correctness to data, host, transfer, compilation, kernels, memory, communication, storage, and hardware, apply symptom playbooks (instability, generalization, throughput, memory, stragglers, checkpoints), confirm with a replay, and report an evidence chain; ships `scan_series.py`, which finds the first sustained robust-z deviation in a metric series and per-rank skew. |

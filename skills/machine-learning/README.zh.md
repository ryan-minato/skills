...
| [training-instrumentation](training-instrumentation/) | 设计训练 run 应输出什么：以共享关联键连接的 outcome、optimization、systems 三层；metrics、阶段级 trace 与定时 profile 各司其职；按优先级给出带采样频率与有界 label 的最小信号集；超越 loss 的模型健康指标与模型族专属信号；相对健康基线、带持续性与复合规则、五级严重度的对症告警；仪表盘；集群规模的采集、采样、保留与容量。 |
| [training-diagnosis](training-diagnosis/) | 诊断异常的 run：跨信号定位第一个异常点，沿自上而下的阶梯依次排查正确性、数据、主机、传输、编译、kernel、内存、通信、存储与硬件，套用按症状分类的手册（数值不稳定、泛化、吞吐、内存、straggler、checkpoint），用重放确认，并给出证据链；附带 `scan_series.py`，用于找出指标序列中第一次持续的 robust-z 偏离以及 rank 间偏差。 |

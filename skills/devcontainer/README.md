...
| [devcontainer-image-prebuild](devcontainer-image-prebuild/) | Prebuild and publish dev container images: devcontainer build --push, the devcontainer.metadata label and its merge semantics with consumer configs, thin consumer configurations, CI automation with devcontainers/ci, cacheFrom layer reuse, and tagging strategy. |
| [devcontainer-template-authoring](devcontainer-template-authoring/) | Create, test, and publish Dev Container Templates: manifest schema and templateOption substitution, payload/option design, the substitute-defaults smoke-test loop, publishing, and a modern repo scaffold with a shared smoke-test action. |

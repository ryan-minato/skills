...
| Skill | Description |
|---|---|
| [agentic-writing](agentic-writing/) | Write, review, and prune the documents an agent reads — entrypoints, knowledge bases, agent-facing spec formats: spend context on behaviour, split by loading rather than topic, word context pointers by their trigger condition, anchor behaviour with leading words, prompt the positive, keep one source of truth per meaning, and prune to the weakest model in scope. |
...
| [great-skill-writing](great-skill-writing/) | Write and improve predictable Agent Skills with trigger-accurate descriptions, progressive disclosure, subagent-assisted behavioral evaluation, isolated script tests, and a bundled linter. |
| [human-writing](human-writing/) | Draft, edit, and review text for human readers in English, Chinese, or Japanese: define type/author/reader/effect first, outline with length budgets, keep the author's stance present, avoid recognizable AI-writing patterns, verify citations, and edit with restraint. |
| [meta-harness](meta-harness/) | Apply the core methodology to every harness-related action: research first, use AGENTS.md as a progressive-loading map, calibrate each layer to the project, close feed-forward and feedback loops, and manage long-term harness entropy. |
| [plan-clarification](plan-clarification/) | Interrogate a plan, an idea, or a decision until agent and user share one understanding: model it as a design tree, ask each round's unblocked questions through the host's own question tool with a recommended answer attached, look every fact up yourself, and leave no files behind. |

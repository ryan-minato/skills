...
| REST fallback scripts in this skill | `GITLAB_TOKEN` (then `GITLAB_ACCESS_TOKEN`), sent as the `PRIVATE-TOKEN` header |
...
script in this skill reuses it, so one variable covers all paths.

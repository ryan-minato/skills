...
Independent review of a commit-backed or bulky GitHub publishing surface.
Input: the scratch directory assembled by the Pre-publish gate — for a PR:
title, body, comments or review text, `commits.txt` from `git log
BASE..HEAD --format=full`, `diff.patch` from `git diff BASE...HEAD`, plus
any attachments; for other bulky surfaces (long or generated release
notes, assets): the assembled files. Output: findings and a binding
verdict line.
...
```
Review the files in <DIR> before they are published to GitHub. Everything
in them will become public — for a PR this includes the branch's commit
messages and diff. You have no other context; judge only what the files
contain.

1. List the files. If the directory is empty, a file the surface requires
   (for a PR: `commits.txt` or `diff.patch`) is missing, or any file is
   unreadable, end with SAFE TO PUBLISH: NO.

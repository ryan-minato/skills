...
| [spec-driven-development](spec-driven-development/) | 以书面规格驱动开发：判断规格驱动开发何时值得、采用哪个级别（spec-first / spec-anchored / spec-as-source），说明各方案家族的适用情形（新应用用 spec-first 套件，软件库、基础设施与任何既有代码用按能力组织的 spec-anchored 变更工作流）并把选择权留给用户，在项目的规格契约之下以工具无关的方式运行 specify–clarify–plan–tasks–implement–verify 循环——发布草稿后停下、门主宣布讨论收口后核对讨论再规划、按契约收敛或存档——没有契约时按文档化的默认值执行，验收标准只保留在规格中，把 vibe-coding 原型或棕地代码库逐个变更地转换过来而不回填规格，并把初始化或改进项目规则的工作交给 `meta` 目录的规格工作流构建技能。 |

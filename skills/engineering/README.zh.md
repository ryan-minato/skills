...
| [spec-driven-development](spec-driven-development/) | 以书面规格驱动开发：判断规格驱动开发何时值得、采用哪个级别（spec-first / spec-anchored / spec-as-source），按情形给出唯一默认方案（新应用用 spec-first 套件，软件库、基础设施与任何既有代码用按能力组织的 spec-anchored 变更工作流）并把决定权留给用户，以工具无关的方式运行 specify–clarify–plan–tasks–implement–verify 循环，敲定规格与工作跟踪的衔接（draft 何时打开、规格是否需要独立的 pull request、审批记录在哪、变更何时由谁存档——附带一个存档已完成 OpenSpec 变更的脚本），验收标准只保留在规格中，并把 vibe-coding 原型或棕地代码库逐个变更地转换过来而不回填规格。 |

...
| [spec-driven-development](spec-driven-development/) | Work from written specifications: judge when spec-driven development pays and at which level (spec-first, spec-anchored, spec-as-source), explain the approach families (a spec-first kit for a new application, a capability-organized spec-anchored change workflow for libraries, infrastructure, and any existing code) and leave the choice to the user, run the specify–clarify–plan–tasks–implement–verify loop tool-neutrally under the project's specification contract — publish the draft and stop, reconcile the discussion when the gate owner closes it and only then plan, converge or archive as the contract says — or its documented defaults when no contract exists, keep acceptance criteria in the spec only, convert a vibe-coded prototype or brownfield codebase one change at a time without backfilling specs, and route setting up or improving the project's rules to the `meta` catalog's spec workflow builder. |
